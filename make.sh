...
setenvs() {
	...
	if [ "$CLOUD_PROVIDER" = "AWS_GOV" ]; then
		...
		export AWS_GOV_SOURCE_AMI=$AWS_GOV_SOURCE_AMI
		export AWS_INSTANCE_TYPE=$AWS_INSTANCE_TYPE
	fi

	export AWS_GOV_AMI_REGIONS="us-gov-west-1,us-gov-east-1"

	# These are shared between AWS + AWS Gov
	...
	export IMAGE_SIZE=$(./scripts/get-image-size.sh $CLOUD_PROVIDER $OS $STACK_VERSION)
	export METADATA_FILENAME_POSTFIX=$METADATA_FILENAME_POSTFIX
	export IMAGE_UUID=$IMAGE_UUID
...
build-azure-redhat8() {
	...
	if [ "$AZURE_INITIAL_COPY" = "true" ]; then
		AZURE_STORAGE_ACCOUNTS=$AZURE_BUILD_STORAGE_ACCOUNT ./scripts/azure-copy.sh
	fi
}
...
push-to-metadata-repo() {
	cleanup-metadata-repo
	./scripts/push-to-metadata-repo.sh
	cleanup-metadata-repo
}
