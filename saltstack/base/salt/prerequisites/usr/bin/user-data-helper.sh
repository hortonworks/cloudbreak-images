...
get_instance_id() {
  ...
  for ((i=1; i<=attempts; i++)); do
    if [[ "$CLOUD_PLATFORM" == "AWS" ]]; then
      local token
      token=$(curl -sf --retry 3 --retry-delay 2 --retry-all-errors \
        -X PUT "http://169.254.169.254/latest/api/token" \
        -H "X-aws-ec2-metadata-token-ttl-seconds: 21600") || true
      if [[ -n "$token" ]]; then
        instance_id=$(curl -sf --retry 3 --retry-delay 2 --retry-all-errors \
          -H "X-aws-ec2-metadata-token: $token" \
          http://169.254.169.254/latest/meta-data/instance-id) || true
      fi
    elif [[ "$CLOUD_PLATFORM" == "AZURE" ]]; then
      instance_id=$(wget -q -O - --tries=3 --wait=2 --retry-connrefused \
