...
openscap_run_stig:
  cmd.run:
    - name: oscap xccdf eval --profile xccdf_org.ssgproject.content_profile_stig --results /tmp/oscap-report.xml /usr/share/xml/scap/ssg/content/ssg-rhel8-ds.xml | tee /tmp/oscap_log.txt

oscap_log_chmod:
  cmd.run:
    chmod 644 /tmp/oscap_log.txt