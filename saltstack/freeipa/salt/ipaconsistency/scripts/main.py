...
- For license see LICENSE on github repo and <http://www.gnu.org/licenses/>

State changes:
- Support JSON & OpenMetrics output format
"""
    ...
    def _print_metrics(self):
        ...
        for key in object:
            ...
            for server_key in server_values:
                ...
                elif metric_val is None or metric_val == "None":
                    metrics_output+="cipa_%s{cipa_server=\"%s\"} -1\n" % (key, server_key)
                else:
                    metrics_output+="cipa_%s{cipa_server=\"%s\", cipa_info=\"%s\"} 0\n" % (key, server_key, metric_val)
