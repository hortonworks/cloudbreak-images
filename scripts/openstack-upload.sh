...
if [[ $OS_KILO_AUTH_URL ]] && [[ $OS_KILO_PASSWORD ]] && [[ $OS_KILO_TENANT_NAME ]] && [[ $OS_KILO_USERNAME ]]; then
    glance image-create --name "$OS_IMAGE_NAME" --file "$OS_IMAGE_NAME.img"  --disk-format qcow2 --container-format bare --os-username "$OS_KILO_USERNAME" --os-tenant-name "$OS_KILO_TENANT_NAME" --os-auth-url "$OS_KILO_AUTH_URL" --os-password "$OS_KILO_PASSWORD"
fi
