...
: ${ARM_CLIENT_ID:?"need to set ARM_CLIENT_ID"}
: ${ARM_CLIENT_SECRET:?"need to set ARM_CLIENT_SECRET"}
: ${ARM_TENANT_ID:?"need to set ARM_TENANT_ID"}
...
IMAGE_VHD=$(docker run -i --rm azuresdk/azure-cli-python:0.2.9 /bin/bash -c "az login --username $ARM_CLIENT_ID --password $ARM_CLIENT_SECRET --service-principal --tenant $ARM_TENANT_ID &> /dev/null; \
...
MANAGED_IMAGE_ID=$(docker run -i --rm azuresdk/azure-cli-python:0.2.9 /bin/bash -c "az login --username $ARM_CLIENT_ID --password $ARM_CLIENT_SECRET --service-principal --tenant $ARM_TENANT_ID &> /dev/null; \
