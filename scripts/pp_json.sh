...
if [ -f package-versions.json -a "$stack_version" != "" -a "$clustermanager_version" != "" ]; then
	...
	if [ "$stack_type" == "CDH" ]; then
		...
		cat package-versions.json | jq --arg stack_version $stack_version --arg clustermanager_version $clustermanager_version '. += {"stack" : $stack_version }' > package-versions-tmp.json && mv package-versions-tmp.json package-versions.json
	fi
	cat package-versions.json
fi
