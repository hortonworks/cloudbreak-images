...
packer_in_container() {
  ...
  if [[ "$INCLUDE_CDP_TELEMETRY" == "Yes" && -z "$CDP_TELEMETRY_RPM_URL" ]]; then
    ...
    CDP_TELEMETRY_RPM_URL="https://cloudera-build-us-west-1.vpc.cloudera.com/s3/build/36082419/cdp-infra-tools/0.x/redhat8/yum/cdp_telemetry-0.4.31.x86_64.rpm"
  ...
  if [[ "$INCLUDE_FLUENT" == "Yes" && -z "$CDP_LOGGING_AGENT_RPM_URL" ]]; then
    ...
    CDP_LOGGING_AGENT_RPM_URL="https://cloudera-build-us-west-1.vpc.cloudera.com/s3/build/36082419/cdp-infra-tools/0.x/redhat8/yum/cdp_logging_agent-0.3.6.x86_64.rpm"
