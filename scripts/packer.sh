...
packer_in_container() {
  ...
  if ! [[ $METERING_AGENT_RPM_URL =~ ^http.*rpm$ ]]; then
    ...
    ## It will be deleted after the proper rpm will be available via the base url
    if [[ "$CLOUD_PROVIDER" == "AWS_GOV" ]]; then
      METERING_AGENT_RPM_URL="https://cloudera-build-us-west-1.vpc.cloudera.com/s3/build/42407979/thunderhead/1.x/redhat8/yum/thunderhead-metering-heartbeat-application-go-1.0.0-b10659.x86_64.rpm"
    else
