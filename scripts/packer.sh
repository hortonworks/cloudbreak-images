...
packer_in_container() {
  ...
  if ! [[ $FREEIPA_PLUGIN_RPM_URL =~ ^http.*rpm$ ]]; then
    ...
    if [[ "$OS" == "redhat8" ]]; then
      export FREEIPA_PLUGIN_RPM_URL="http://cloudera-build-us-west-1.vpc.cloudera.com/s3/build/48919567/thunderhead/1.x/redhat8/yum/cdp-hashed-pwd-1.0-20240104200644git7e13294.el8.x86_64.rpm"
    else
