...
function highstate {
  ...
  salt-call --no-color --local state.highstate saltenv=${saltenv} -l info --log-file=/tmp/salt-build-${saltenv}.log --log-file-level=info --config-dir=/tmp/saltstack/config
}
...
function apply_optional_states {
  ...
  then
    ...
    copy_resources ${saltenv}
    salt-call --no-color --local state.sls ${OPTIONAL_STATES} saltenv=${saltenv} pillarenv=${saltenv} -l info --log-file=/tmp/salt-build-${saltenv}.log --config-dir=/tmp/saltstack/config
  fi
}
