...
if [[ ( x"$OS"x == x"Ubuntu"x && x"$OS_VERSION"x == x"16.04"x ) ]]; then
    ...
    exec -l /bin/systemd --system
    ############################### SLES12 #####################
elif [[ ( x"$OS"x == x"CentOS"x && x"$OS_VERSION"x == x"7.2.1511"x ) || ( x"$OS"x == x"CentOS"x && x"$OS_VERSION"x == x"7.3.1611"x ) || ( x"$OS"x == x"CentOS"x && x"$OS_VERSION"x == x"7.4.1708"x ) || ( x"$OS"x == x"CentOS"x && x"$OS_VERSION"x == x"7.5.1804"x ) || ( x"$OS"x == x"CentOS"x && x"$OS_VERSION"x == x"7.6.1810"x ) || (  x"$OS"x == x"CentOS"x && x"$OS_VERSION"x == x"7.9.2009"x ) ]]; then
    ############################### Centos7 #####################
    # Centos7 has systemd
    systemctl enable sshd
    exec -l /usr/lib/systemd/systemd --system
    ############################### Centos7 #####################
elif [[ ( x"$OS"x == x"RedHat"x && x"$OS_VERSION"x == x"8"x )  ]]; then
    ############################### RedHat8 #####################
